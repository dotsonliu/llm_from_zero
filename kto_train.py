...
class DpoConfig:
    ...
    sft_model_file: str = '/home/caslx/scalinglaw/MINI_LLM/model_save/sft/checkpoint-42000' # SFT后的模型路径
    tokenizer_dir: str = '/home/caslx/scalinglaw/MINI_LLM/model_save/sft/checkpoint-42000'   # tokenizer一般和model权重放在同一个文件夹

    dpo_train_file: str = '/home/caslx/scalinglaw/MINI_LLM/datasets/dpo/my_kto_train.json' # dpo的训练集
    dpo_eval_file: str = '/home/caslx/scalinglaw/MINI_LLM/datasets/dpo/my_kto_eval.json' # dpo的测试集

    adapter_file: str = './dpo/adapter_model.safetensors'
    log_dir: str = './ktologs'
    ...
    save_steps: int = 2000
    output_dir: str = './ktosavemodel'  # dpo模型输出路径
    warmup_steps: int = 1000
    fp16: bool = True
    seed: int = 23333
    beta: float = 0.1
import json
def get_dataset(orgpath):
        with open(orgpath,"r+",encoding='utf-8') as f:
            r = json.load(f)
            allresults = {}
            allresults["prompt"] = []
            allresults["completion"] = []
            allresults["label"] = []

            for tmp in r["prompt"]:
                allresults["prompt"].append(tmp+'<|im_end|>')
            for tmp in r["completion"]:
                allresults["completion"].append(tmp+'<|im_end|>')
            allresults["label"] = r["label"]
            return Dataset.from_dict(allresults)
...
#             "completion": f"{sample['completion']}<|im_end|>",
#             "label":sample["label"]
#         }
#         return sentence1           
...
def train_dpo(config: DpoConfig, peft_config: LoraConfig=None) -> None:
    ...
    # 4. 加载训练数据集
    train_dataset = get_dataset(config.dpo_train_file)

    # 5. 加载评估数据集
    # eval_dataset = get_dataset("train", file=config.dpo_eval_file)
    eval_dataset = get_dataset(config.dpo_eval_file)
    ...
    if not os.path.exists(log_dir):
        os.mkdir(log_dir)
    loss_log.to_csv(f"{log_dir}/kto_train_log_{time.strftime('%Y%m%d-%H%M')}.csv")
    
    # 10. 保存模型/lora
    suffixe = '/lora/' if peft_config is not None else '/kto'
