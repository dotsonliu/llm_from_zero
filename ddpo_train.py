...
os.environ["WANDB_PROJECT"]="rewarm"
...
class DpoConfig:
    ...
    dpo_train_file: str = r'/home/caslx/scalinglaw/MINI_LLM/datasets/dpo/my_dpo_train.json' # dpo的训练集
    dpo_eval_file: str = r'/home/caslx/scalinglaw/MINI_LLM/datasets/dpo/my_dpo_eval.json' # dpo的测试集

    adapter_file: str = './orpo/adapter_model.safetensors'
    log_dir: str = './orpologs'
    ...
    save_steps: int =1000
    output_dir: str = './orposavemodel'  # dpo模型输出路径
...
def get_dataset(orgpath):
    ...
    with open(orgpath,"r+",encoding='utf-8') as f:
        r = json.load(f)
        for tmpline in r:
            allresults["prompt"].append(tmpline["prompt"]+'<|im_end|>')
            allresults["chosen"].append(tmpline["chosen"]+'<|im_end|>')
            allresults["rejected"].append(tmpline["rejected"]+'<|im_end|>')
...
def train_dpo(config: DpoConfig, peft_config: LoraConfig=None) -> None:
    ...
    # 10. 保存模型/lora
    suffixe = '/lora/' if peft_config is not None else '/ddpo'
